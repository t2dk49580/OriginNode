...
void toHex(char *dest,const uint8_t* source,int sourceLen)
{
    short i;
...
int toBin( uint8_t* pDst, const uint8_t* pSrc, int nSrcLength )
{
...
uint8_t pubickey[128] = {0};
uint8_t pubicsign[128] = {0};

char * getPublicKey(){
    char *p = (char *)pubickey;
    return p;
}
char * getPrivateKey(){
    char *p = (char *)privatekey;
    return p;
}
char * getSign(){
    char *p = (char *)pubicsign;
    return p;
}
...
int getkey() {
    ...
    if (!uECC_make_key(pubkey, prikey, curves)) {
        printf("uECC_make_key() failed\n");
        return 0;
    }
    toHex(privatekey,prikey,32);
    toHex(pubickey,pubkey,64);
    return 1;
}
...
int sign(char *pkey, char *msg) {
    ...
    toBin(prikey,pkey,strlen(pkey));
    toBin(hash,msg,strlen(msg));

    const struct uECC_Curve_t *curves = uECC_secp256k1();

    if (!uECC_sign(prikey, hash, 32, sig, curves)) {
        printf("uECC_sign() failed\n");
        return 0;
    }
    toHex(pubicsign,sig,64);
    return 1;
}

int verify(char *pkey,char *msg,char *s) {
    uint8_t pubkey[64] = {0};
    uint8_t sig[64] = {0};
    uint8_t hash[32] = {0};

    toBin(pubkey,pkey,128);
    toBin(hash,msg,64);
    toBin(sig,s,128);

    const struct uECC_Curve_t *curves = uECC_secp256k1();

    if (!uECC_verify(pubkey, hash, 32, sig, curves)) {
